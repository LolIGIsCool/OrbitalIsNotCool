...
function zclib.Inventory.ApplyItemVisuals(ent,SlotData)
    ...
    if SlotData.Model then
        ent:SetModel(SlotData.Model)
    end

    if SlotData.Skin then
        ent:SetSkin(SlotData.Skin)
    end

    if SlotData.Material then
        ent:SetMaterial(SlotData.Material)
    end

    if SlotData.Color then
        ent:SetColor(SlotData.Color)
    end

    if SlotData.BodyG then
        for k, v in pairs(SlotData.BodyG) do
