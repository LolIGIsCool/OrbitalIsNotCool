...
net.Receive("zclib_Inventory_Sync", function(len)
    ...
    if inv and IsValid(zclib_main_panel) and (ent == zclib.vgui.ActiveEntity or ent == LocalPlayer()) and zclib_main_panel.OnInventoryChanged then
...
function zclib.Inventory.SlotOptions(pnl,slot)
    ...
    if pnl.Inventory.SlotOptions then
        ...
        if (ItemDefinition == nil or ItemDefinition.PreventDrop == nil) and pnl.Inventory.inv_ent ~= LocalPlayer() then
