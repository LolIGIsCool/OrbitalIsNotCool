...
vgui.Register('os_menu_playerlabel', PANEL, 'ui_panel')
...
function PANEL:GetValue()
	if (self.Value == nil) or (self.Value == nil) then
		return nil
	end
...
vgui.Register('os_menu_timeselect', PANEL, 'ui_panel')
...
vgui.Register('os_menu_player', PANEL, 'ui_panel')
...
vgui.Register('os_menu_command', PANEL, 'ui_panel')
...
vgui.Register('os_menu_commandargs', PANEL, 'ui_panel')
...
vgui.Register('os_menu_checkbox', PANEL, 'DCheckBoxLabel')
...
function PANEL:Init()
	...
	self.Settings:SetSize(surface.GetTextSize('Settings') + 5, 27)
	self.Settings:SetPos(0, 4)

	self.Settings.Paint = function(s, w, h)
		if self.Settings.Hovered then
			draw.Box(10, h - 8, w - 5, 2, ui.col.SUP)
		end
	end
	...
	for k, v in pairs(cvar.GetTable()) do
		local title = (v:GetMetadata('osMenu') or v:GetMetadata('Menu'))
		if title then
			ui.Create('os_menu_checkbox', function(self, p)
				self:SetText(title)
				self:SizeToContents()
	...
	self:SetSize(ScrW() * .275, ScrH() * .425)
	self:Center()
	self:MakePopup()
end

vgui.Register('os_menu_settings', PANEL, 'ui_frame')
...
function PANEL:Init()
	...
	self.lblTitle:SetText(' Orbital Servers')
	...
	for k, v in ipairs(player.GetAll()) do
		local pl = ui.Create('os_menu_player')
		pl:SetPlayer(v)
	...
	for k, v in pairs(os.cmd.GetTable()) do
		if PlayerCMD(v:GetArgs()) then
			local c = ui.Create('os_menu_command')
			c:SetCommand(v)
...
function PANEL:PerformLayout()
	...
	surface.SetFont('ui.22')
	self.Settings:SetSize(surface.GetTextSize(' Jims') + 5, 27)
	self.Settings:SetPos(5, 4)
...
function PANEL:CreateCont(c)
	self.Cont = ui.Create('os_menu_commandargs', self)
...
vgui.Register('os_menu', PANEL, 'ui_frame')



function os.ui.OpenMenu()
	if IsValid(Menu) then Menu:Remove() end
	Menu = ui.Create('os_menu')
end

function os.ui.AddCVar(cvar, title)
	os.ui.Settings[cvar] = title
end

function os.ui.OpenSettings()
	if IsValid(Settings) then Settings:Remove() end
	Settings = ui.Create('os_menu_settings')
end
